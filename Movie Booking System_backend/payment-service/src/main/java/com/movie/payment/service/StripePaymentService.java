...
//    }
    ...
    public PaymentResponseDto createStripeCheckoutSession(PaymentRequestDto request) {
        try {
            long amountInCents = request.getAmount().multiply(new BigDecimal(100)).longValueExact();
                                ...
                                SessionCreateParams.LineItem.PriceData.builder()
                                    ...
                                    .setProductData(
                                        SessionCreateParams.LineItem.PriceData.ProductData.builder()
                                            .setName("Booking #" + request.getBookingId())
                                            .build()
                                    )
                                    .build()
                            )
            ...
            log.error("Stripe API error while creating checkout session for booking id {}: {}", request.getBookingId(), e.getMessage(), e);
            return PaymentResponseDto.builder()
                    .bookingId(request.getBookingId())
                    .status(PaymentResponseDto.PaymentStatus.FAILED)
                    .message("Stripe API error: " + e.getMessage())
